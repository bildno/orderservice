    ...
    protected void doFilterInternal(HttpServletRequest request,
        ...
        try {
            ...
            if(token != null) {
                ...
                // 나중에 컨트롤러의 요청 메서드마다 권한을 파악하게 하기 위해 미리 저장을 해놓는 것
                List<SimpleGrantedAuthority> authorityList = new ArrayList<>();
                // ROLE_USER, ROLE_ADMIN (ROLE_ 접두사는 필수)
...
//                response.getWriter().write("토큰이 업따요");
//            }
            }
            //필터를 통과하는 메서드 (dofilter안하면 필터 통과 못함)
            ...
            response.setStatus(HttpStatus.UNAUTHORIZED.value());
            response.setContentType("application/json");
            response.getWriter().write("토큰에 문제가 있따");
        }
    ...
    private String parseBearerToken(HttpServletRequest request) {
        ...
        // 아직 순수 토큰값이 아닌 Bearer " " 이 붙어 있으니 이것을 제거
